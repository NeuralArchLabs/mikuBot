...
/** Emoji shortcode map */
const emojiAnimations_1 = require("../animations/emojiAnimations");
 ...
 */
const toHtml = (md, isStreaming = false, mode = 'full') => {
    if (!md)
        return '';
    ...
    html = html.replace(/"?\{\{\s*([^\}]+?)\s*\}\}"?/g, (match, signContent) => {
        if (signContent.includes('≈') || signContent.includes('∫') || signContent.includes('~')) {
            ...
            styledInner = styledInner.replace(/(\p{Emoji_Presentation}|\p{Extended_Pictographic})/gu, (emojiMatch) => {
                const animClass = (0, emojiAnimations_1.getEmojiAnimationClass)(emojiMatch);
    ...
    /[`"']*(?:\{\{)?\s*[`"']*\s*((?:\p{Emoji_Presentation}|\p{Extended_Pictographic}|\uFE0F|\u200D|\uFE0E)*)\s*[`"']*\s*(≈̼\^\.┬\.̼\^≈‿⟆)\s*[`"']*\s*((?:\p{Emoji_Presentation}|\p{Extended_Pictographic}|\uFE0F|\u200D|\uFE0E)*)\s*[`"']*\s*(?:\}\})?[`"']*/gu, (fullMatch, leadEmojis, core, trailEmojis) => {
        ...
        const normalizeEmoji = (s) => s.replace(/[\uFE0E\uFE0F]/g, '').trim();
            ...
            if ((tagName === 'div' || tagName === 'section' || tagName === 'article' || tagName === 'aside' || tagName === 'main' || tagName === 'figure' || tagName === 'details' || tagName === 'summary') && fullTagContent.includes('\n')) {
                ...
                if (closeTagIdx > openTagEnd) {
                    ...
                    inner = inner.replace(/^(#{1,6})\s+(.+)$/gm, (m, hashes, text) => {
                        ...
                        if (lvl === 1)
                            return `<h1 class="text-lg font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-indigo-300 drop-shadow-[0_1.5px_2px_rgba(0,0,0,0.8)] mt-6 mb-1">${text}</h1><div class="h-px w-full" style="background: linear-gradient(to right, transparent 0%, rgba(34,211,238,0.15) 2%, rgba(34,211,238,0.15) 98%, transparent 100%); margin-bottom: 1.5rem;"></div>`;
                        if (lvl === 2)
                            return `<h2 class="text-md font-bold text-cyan-400 drop-shadow-[0_1.5px_2px_rgba(0,0,0,0.8)] mt-5 mb-1">${text}</h2><div class="h-px w-full" style="background: linear-gradient(to right, transparent 0%, rgba(255,255,255,0.08) 2%, rgba(255,255,255,0.08) 98%, transparent 100%); margin-bottom: 1rem;"></div>`;
                        if (lvl === 3)
                            return `<h3 class="text-sm font-bold text-[#FC8F35] drop-shadow-[0_1.5px_2px_rgba(0,0,0,0.8)] mt-4 mb-2">${text}</h3>`;
                        if (lvl === 4)
                            return `<h4 class="text-sm font-bold text-[#fca865] drop-shadow-[0_1.5px_2px_rgba(0,0,0,0.8)] mt-4 mb-2">${text}</h4>`;
                        if (lvl === 5)
                            return `<h5 class="text-xs font-bold text-indigo-400/70 mt-3 mb-1">${text}</h5>`;
    ...
    const isMathHeuristic = (formula, isDollar = false) => {
    ...
    html = html.replace(emojiCalloutRegex, (_match, emoji, rest) => {
    ...
    html = html.replace(/^>\s*\*{1,2}\s*([^*\n:：]+?)\s*\*{1,2}\s*[:：]?\s*$/gm, (_match, rawKeyword) => {
    ...
    html = html.replace(spanishCalloutRegex, (_match, type) => {
    ...
    html = html.replace(/^[ \t]*(?:>\s*)?\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION|DANGER|INFO|SUCCESS|FAILURE|BUG|EXAMPLE|QUOTE|QUESTION|FAQ|SECURITY|TODO|REMEMBER|CHECK|ABSTRACT)\]([\-\+])?(?:[ \t]+(.*))?\s*?\n?((?:(?!(?:[ \t]*>\s*\[!)).*\n?)*)/gim, (match, type, collapseSign, title, body) => {
        ...
        const localizedDefault = CALLOUT_I18N_KEYS[typeUp] ? i18n_1.default.t(`common.${CALLOUT_I18N_KEYS[typeUp]}`) : typeUp;
        ...
        const bodyHtml = content ? `<div class="text-md font-medium text-slate-300 ${isCollapsible ? 'mt-3 pt-3 border-t border-white/5' : 'leading-relaxed'} child-content typing-content">${(0, exports.toHtml)(content, isStreaming)}</div>` : '';
        if (isCollapsible) {
    ...
    html = html.replace(/&lt;p\s+align="(center|right|left)"&gt;(.*?)&lt;\/p&gt;/gi, (_, align, content) => `<div class="text-slate-200" style="text-align:${align}">${content}</div>`);
 ...
 * Robust implementation that handles missing outer pipes and malformed headers.
 */
function convertTablesToHtml(html) {
    ...
    while (i < lines.length) {
        ...
        // TABLE DETECTION: Look for a potential header followed by a separator
        const hasPipes = trimmed.includes('|');
        const nextLine = (lines[i + 1] || '').trim();
        const nextHasPipes = nextLine.includes('|');
        const isNextSeparator = nextHasPipes && /^[|:\s\-\+]{3,}$/.test(nextLine);
        // A table starts if we have pipes and either a leading pipe OR a separator following
        if (!inPre && hasPipes && (trimmed.startsWith('|') || isNextSeparator)) {
            let currentTable = [];
            let alignments = [];
            while (i < lines.length) {
                const currentLine = lines[i];
                const currentTrimmed = currentLine.trim();
                // Handle blank lines inside a table block:
                // Skip blank lines if the NEXT non-blank line still has pipes (still in table)
                if (currentTrimmed === '') {
                    let nextNonBlank = '';
                    ...
                    // If the next non-blank line has pipes, we're still in the table — skip this blank line
                    if (nextNonBlank.includes('|')) {
                        i++;
                        continue;
                    }
                    // Otherwise, the table truly ended
                    break;
                }
                // End table if no pipes and not a separator
                if (!currentTrimmed.includes('|') && !/^[|:\s\-\+]{3,}$/.test(currentTrimmed)) {
                    break;
                }
                // Check for separator row
                const isSeparator = currentTrimmed.includes('|') && /^[|:\s\-\+]{3,}$/.test(currentTrimmed);
                if (isSeparator) {
                    const parts = currentTrimmed.split('|').filter(p => p.trim() !== '');
                ...
                const cleanLine = currentTrimmed.replace(/^\|/, '').replace(/\|$/, '');
...
function renderTable(rows, alignments = []) {
    if (rows.length === 0)
        return '';
    let maxCols = 0;
    rows.forEach(r => { if (r.length > maxCols)
        maxCols = r.length; });
    ...
    for (let i = 0; i < maxCols; i++) {
        ...
        const processedText = processInlineMarkdown(cellText);
        ...
        html += `<th class="px-4.5 py-3 text-${align} text-[12px] font-black text-white uppercase tracking-[0.15em] whitespace-nowrap relative ${borderX}">`
            + `<span class="relative z-10">${processedText}</span>`
 ...
 */
function convertBlockquotesToHtml(block, isStreaming = false) {
    ...
    const processAtDepth = (inputLines, targetDepth) => {
        const output = [];
        let i = 0;
...
function convertListsToHtml(html) {
    ...
    const getLiStyle = (d) => liDepthStyles[Math.min(d, liDepthStyles.length - 1)];
...
function highlightCode(code, lang) {
    ...
    const tokens = [];
    const addToken = (content, className) => {
    ...
    // At start of line
    highlighted = highlighted.replace(/^(\s*)(\/\/|#)(.*)$/gm, (match, space, prefix, content) => space + addToken(prefix + content, 'text-slate-600 italic font-medium'));
    // Inline (preceded by space)
    highlighted = highlighted.replace(/([ \t]+)(\/\/|#)(.*)$/gm, (match, space, prefix, content) => space + addToken(prefix + content, 'text-slate-600 italic font-medium'));
    if (lang === 'mermaid' || lang === 'flowchart' || lang === 'graph' || lang === 'gitgraph' || lang === 'erdiagram' || lang === 'mindmap' || lang === 'pie' || lang.startsWith('statediagram') || lang === 'gantt' || lang === 'sequencediagram') {
        ...
        highlighted = highlighted.replace(/(:\s*)(\d+(\.\d+)?|[\d\-]{4,})/g, (_, colon, val) => `${colon}${addToken(val, 'text-amber-400 font-black')}`);
        // 3. Labels in brackets - (e.g., [Text] or {Text})
        highlighted = highlighted.replace(/([\[\(\{])([^\]\)\}]*)([\]\)\}])/g, (_, open, content, close) => `${addToken(open, 'text-slate-500')}${addToken(content, 'text-slate-200 font-semibold')}${addToken(close, 'text-slate-500')}`);
        ...
        highlighted = highlighted.replace(/([\w\-_]+\/)/g, (match) => addToken(`<i class="fas fa-folder text-amber-500/90 mr-1.5"></i>${match}`, 'text-amber-200/90 font-bold'));
        // 3. Files (names with extensions)
        highlighted = highlighted.replace(/([\w\-_]+\.(?:ts|js|json|md|py|css|html|tsx|jsx|env|cjs|mjs|txt))/g, (match) => addToken(`<i class="far fa-file-code text-blue-400/80 mr-1.5"></i>${match}`, 'text-slate-200'));
    }
