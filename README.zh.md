...
*   **MikuBot Markdown Engine (Studio Elite):** MikuBot 集成了一个专业级渲染引擎，专为技术主权和技术精确度而设计。此 "Studio Elite" 套件重新定义了视觉交互：
    *   **科学渲染 (LaTeX):** 高性能原生支持复杂数学公式 ($$ 和 $)，包括积分、矩阵、二项式系数和物理常数，具有完美的视觉对齐。
    *   **智能 Mermaid 图表:** 流畅可视化 `flowcharts`、`gitGraphs`、`mindmaps` 和 `erDiagrams`，具有平滑的展开动画和实时语法错误管理。
    *   **Obsidian 风格提示框 (Callouts):** 通用支持 `> [!TYPE]`（可折叠和展开），具有超过 14 种主题反应样式和同步的**打字动画**。
    *   **动力学画布:** 神经签名和表情符号的有机动画系统，每个符号都根据其视觉身份遵循独特的模式（跳动、脉冲、闪烁）。
    *   **隐私控制 (剧透):** 高级磨砂玻璃剧透 (`||spoiler||`)，在交互时显示青色光芒。
    *   **动态跟踪 (进度条):** 通过带有自动颜色级别的交互式栏 (`[====]`, `[████]`) 可视化状态和进度。
    *   **资产保护架构:** 一个 3 阶段管道，确保图像、链接和嵌套代码块在 HTML 处理过程中永远不会丢失其结构。*   **神经编辑 (Cortex & Command Editors)：** MikuBot 包括专门的编辑器，可直接修改助手的指令和基础知识。虽然您可以随时调整这些文件进行技术自定义，但**建议遵循入门向导 (Onboarding Wizard) 流程**以获得最佳效果。像 `MODES.md` 这样的关键文件决定了代理的操作协议，手动更改需要谨慎以保持系统稳定性。
...
**第三方致谢:**
MikuBot 的实现归功于 React (MIT)、Vite (MIT)、Electron (MIT)、Tailwind CSS (MIT)、Zustand (MIT)、FastAPI (MIT) 以及 Ollama 和 Google GenAI (Apache 2.0) 等 AI SDKs。有关更多详细信息，请参阅 [`LICENSE`](./LICENSE) 文件。
