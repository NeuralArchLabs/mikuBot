...
*   **MikuBot Markdown Engine (Studio Elite):** MikuBot integrates a professional-grade rendering engine designed for technological sovereignty and technical precision. This "Studio Elite" suite redefines visual interaction:
    *   **Scientific Rendering (LaTeX):** High-performance native support for complex mathematics ($$ and $), including integrals, matrices, binomial coefficients, and physical constants with perfect visual alignment.
    *   **Intelligent Mermaid Diagrams:** Fluid visualization of `flowcharts`, `gitGraphs`, `mindmaps`, and `erDiagrams` with smooth expansion animations and real-time syntax error management.
    *   **Obsidians-Style Callouts:** Universal support for `> [!TYPE]` (collapsible and expanded) with over 14 theme-reactive styles and synchronized **typing animations**.
    *   **Kinetic Canvas:** Organic animation system for neural signatures and emojis, where every symbol follows a unique pattern (bounce, pulse, shimmer) based on its visual identity.
    *   **Privacy Control (Spoilers):** Premium frosted-glass spoilers (`||spoiler||`) that reveal with a cyan glow upon interaction.
    *   **Dynamic Tracking (Progress Bars):** Visualization of states and progress via interactive bars (`[====]`, `[████]`) with automated color levels.
    *   **Asset Protection Architecture:** A 3-phase pipeline ensuring that images, links, and nested code blocks never lose their structure during HTML processing.*   **Neural Editing (Cortex & Command Editors):** MikuBot includes specialized editors to directly modify the assistant's instructions and base knowledge. While you can adjust these files at any time for technical customization, **it is recommended to follow the Onboarding Wizard process** for optimal results. Critical files like `MODES.md` dictate the agent's operating protocols, and manual changes require caution to maintain system stability.
...
MikuBot is made possible thanks to open-source technologies like React (MIT), Vite (MIT), Electron (MIT), Tailwind CSS (MIT), Zustand (MIT), FastAPI (MIT), and AI SDKs like Ollama and Google GenAI (Apache 2.0). Refer to the [`LICENSE`](./LICENSE) file for more details.
