...
Un asistente y agente de IA con verdadera alma, diseñado para ser el acompañante perfecto en Windows. Nos alejamos de los "tableros grises estériles" que dominan el mercado para ofrecer una experiencia visual viva, inmersiva y moderna. Su diseño y funcionalidad están meticulosamente optimizados para todo público: desde niños que buscan un compañero de IA para aprender y ser guiados, personas mayores sin conocimientos técnicos que necesitan asistencia sencilla, hasta *pro-users* y oficinistas que requieren un entorno robusto y herramientas avanzadas. mikuBot no sigue estándares convencionales; forjamos nuestro propio camino para no ser "un clon más".
...
*   **mikuBot Markdown Engine (Studio Elite) & HTMDL:** Nuestro motor de renderizado "Studio Elite" reescribe las reglas utilizando una propuesta única a la que llamamos **HTMDL** (mezcla magistral de HTML y Markdown). El modelo utiliza el HTMDL de forma nativa y proactiva como su *metaherramienta* principal (sin requerir llamadas a funciones externas) para estructurar datos, creando presentaciones visuales sin igual que ningún otro agente o cliente web posee. *(Estamos trabajando en publicar las especificaciones de implementación de HTMDL para que la comunidad pueda explorar y adoptar esta visión).* Esta suite redefine la interacción visual:
    ...
    *   **Arquitectura y Lógica de Datos (Mermaid):** Visualización nativa de diagramas de flujo, GitGraphs y mapas mentales directamente en el hilo de conversación.
...
- **Investigación Web (`web_search`, `web_search_more`, `read_url`):** Búsqueda paginada, extracción selectiva y lectura directa de fuentes.
- **Investigación Profunda (`deep_research`):** Motor de investigación planificada y auditable para evitar alucinaciones y obtener datos técnicos precisos.
...
| :--- | :--- | :--- | :--- | :--- |
| **Paradigma e Interfaz** | **Híbrido (Desktop + 24/7).** Control visual total con capacidad de ejecución persistente en segundo plano y vinculación con Telegram. | **Always-on / Autonomous.** Asistente persistente que opera 24/7 como un servicio de fondo. | **Daemon Headless / Mensajería.** Se controla a través de WhatsApp, Telegram o la terminal (TUI). | **Bot Empresarial para Equipos.** Integrado principalmente en Slack, Discord o Feishu. |
...
| **Razonamiento Visual** | **HTMDL y Neural Flow.** Streaming visual de pensamientos y mezcla de HTML/Markdown. Una UX viva con verdadera "alma", sin tableros grises estériles. | **Logs / Mensajería.** Interacción a través de interfaces de chat externas. | **Logs de Texto.** Salida cruda de terminal sin representación visual del estado interno. | **Historial Estándar.** Interfaz de chat convencional sin desglose técnico visual. |
...
| **Visualización** | **HTMDL (Studio Elite).** Mezcla activa de HTML/Markdown que le da "alma" al chat, alejándose de los diseños estériles y clones del mercado. | **Web UI Estándar.** Interfaz limpia y funcional, pero tradicional. | **Markdown Básico.** Tableros grises convencionales de chat sin flexibilidad técnica. |
