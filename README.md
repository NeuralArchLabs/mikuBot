...
*   **MikuBot Markdown Engine (Studio Elite):** MikuBot integra un motor de renderizado de grado profesional diseñado para la soberanía tecnológica y la precisión técnica. Esta suite "Studio Elite" redefine la interacción visual:
    *   **Renderizado Científico (LaTeX):** Soporte nativo de alto rendimiento para matemáticas complejas ($$ y $), incluyendo integrales, matrices, coeficientes binomiales y constantes físicas con alineación visual perfecta.
    *   **Diagramas Mermaid Inteligentes:** Visualización fluida de `flowcharts`, `gitGraphs`, `mindmaps` y `erDiagrams` con animaciones de expansión suave y gestión de errores de sintaxis en tiempo real.
    *   **Callouts estilo Obsidian:** Soporte universal para `> [!TYPE]` (colapsables y expandidos) con más de 14 estilos reactivos al tema y animaciones de flujo (*typing animation*) sincronizadas.
    *   **Canvas Cinético:** Sistema de animaciones orgánicas para la firma neural y emojis, donde cada símbolo sigue un patrón único (rebote, pulso, brillo) basado en su identidad visual.
    *   **Control de Privacidad (Spoilers):** Implementación de spoilers de cristal esmerilado (`||spoiler||`) que se revelan con un halo cian al interactuar.
    *   **Seguimiento Dinámico (Progress Bars):** Visualización de estados y progreso mediante barras interactivas (`[====]`, `[████]`) con niveles de color automáticos.
    *   **Arquitectura de Protección de Assets:** Pipeline de 3 fases que garantiza que imágenes, links y bloques de código anidados nunca pierdan su estructura durante el procesamiento de HTML.*   **Edición Neural (Cortex & Command Editors):** MikuBot incluye editores especializados para modificar directamente las instrucciones y el conocimiento base del asistente. Aunque puedes ajustar estos archivos en cualquier momento para una personalización técnica, **se recomienda seguir el proceso del Onboarding Wizard** para obtener resultados óptimos. Archivos críticos como `MODES.md` dictan los protocolos de operación del agente y su manipulación manual requiere precaución para mantener la estabilidad del sistema.
