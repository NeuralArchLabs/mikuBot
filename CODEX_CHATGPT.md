...
- Codex recibe además `summary: detailed` cuando el razonamiento está activo para solicitar el resumen legible que llega por `item/reasoning/summaryTextDelta`. Con `none` se envía `summary: none`. Esto muestra un resumen del razonamiento, no la cadena privada de pensamiento completa.

La habilidad Python **Deep Research** tiene su propio transporte de modelos y todavía no admite Codex. El chat, el modo Agente y sus herramientas habituales usan esta integración; Deep Research requiere uno de sus proveedores compatibles.
